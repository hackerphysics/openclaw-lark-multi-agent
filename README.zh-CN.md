...
最终回复不会覆盖旧的 live status，而是在任务完成时新发一张卡片，因此用户执行途中插入的新消息不会排在最终答案之后。最终回复卡片底部只显示本次使用的模型名；工具调用次数和耗时只保留在 live status 中。
...
状态与上下文已用量/上限来自同一次、有等待上限的 Gateway 查询；K 按
1000 token 四舍五入为整数。缺失或明确过期的数据不冒充 0。尾部反映发送前
查询的状态，不会持续自动刷新历史回复。

前台等待改为**连续 10 分钟没有有效任务活动才暂停**，不是任务总时长上限。
已归属本次任务的新 assistant 文本、不同工具调用的开始/结束等会重置 collector
的静默计时；卡片 elapsed ticker、状态轮询、usage-only 与重复事件不会续时。
卡片没有独立总时长计时器；chat.send 接受及 collector 启动前的排队/内部耗时不算
任务静默。9:35 的 write 完成后，最早 19:35 静默到期，不能在 10:00 冻结。
到期发送普通等待提示，显示实际状态（不可用也不冒充 running），不判任务失败。
真正的 chat-final `yielded: true` 仍立即提示并冻结，不等 10 分钟。
普通静默暂停与 yield 都沿用已发布策略：后续工具活动不会自动解冻旧卡。
后台仍监听原任务，保留原队列，不因此停止或自动重发请求。真实结果另行回复，
保留必要的终态收尾。本改动不缩短 Gateway 的执行时限，也不包含未验收的
steer V2 与问答适配器。

计时、API 参数及限制详见 [前台静默计时说明](docs/foreground-idle-2026-09-11.md)。
