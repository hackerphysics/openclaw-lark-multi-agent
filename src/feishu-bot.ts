...
只有明确 @ 我才会回复`);
    ...
    while (true) {
      ...
      try {
        ...
        try {
          ...
          // only auto-retry replies that look truncated, not transient run errors.
          throw mainErr;
        } finally {
   ...
   *
   * This is the ONLY auto-retry trigger: a reply that LOOKS UNFINISHED. We do not
   * retry on transient run errors, and we do not auto-compact-and-retry.
   */
  private async autoRetryUntilComplete(params: {
    ...
    for (let attempt = 1; attempt <= maxRetry; attempt++) {
      ...
      if (wasStopped()) {
        ...
        return probeText && probeText.toUpperCase() !== "NO_REPLY" ? probeReply : reply;
      }
